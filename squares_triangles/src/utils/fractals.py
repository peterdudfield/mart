...
class Fractals(object):
    ...
    def make_shape_from_verticie(self, p1, p2, r, overall_level, shape_level):
        ...
        rotation = ((-1) ** (self.sides - 1)) * (r + self.sides - 3) * (
            np.pi * 2 / self.sides
        ) + np.pi * (shape_level % 2)
    ...
    def make_shapes_verticles(self, layer_str, verticles, color, layer):
        ...
        for i in range(0, len(verticles)):
            ...
            shapes.append(
                self.make_shape_from_verticie(
                    verticles[i][0], verticles[i][1], i, layer, verticles[i][2]
                )
            )
    ...
    def make_the_image(self, first_shape):
        ...
        for i in range(self.N_layers):
            shapes_tmp, verticles = self.make_shapes_verticles(
                str(i + 2), verticles, self.colors[i + 1], i + 2
            )
