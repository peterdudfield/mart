...
def make_lines(layer_str,lines,layer,L_d,R_d,Theta1,Theta2,scalefactor):
  ...
  for i in range(0,len(lines)):
    ...
    new_points = make_points_from_line(line[0],line[1],layer,L_d,R_d,Theta1,Theta2,scalefactor)
...
def make_tree(Theta1, Theta2,L_d,R_d,size,scalefactor=2,N_layers=6):
    #first triangles
    ...
    for i in range(N_layers+1):
        ...
        for line in lines_layer[i]:
            ...
            if i>4:
                ...
                linewidth = 2
            plt.plot([line[0][0],line[1][0]],[line[0][1],line[1][1]],make_color(i),
...
print('Making tree 1')
make_tree(np.pi/3*0.8, np.pi/3*0.9,0.3,0.2,1.4,N_layers=min_N_layers+1)

ax = fig.add_subplot(220+2)
print('Making tree 2')
make_tree(np.pi/3*0.8, np.pi/3*0.8,0.7,0.7,2,scalefactor=1.6,N_layers=min_N_layers+3)

ax = fig.add_subplot(220+3)
print('Making tree 3')
make_tree(np.pi/4*0.8, np.pi/3*1.2,0.5,0.5,1.5,scalefactor=1.9,N_layers=min_N_layers+1)

ax = fig.add_subplot(220+4)
print('Making tree 4')
make_tree(np.pi/4*1.2, np.pi/4*1.1,0.4,0.8,1.5,N_layers=min_N_layers+1)
